...
## Load Data
We will extract data from the Urls provided for this assigment.
```{r load data}
...
## Split Data
Now we have to split our dataset into train and test set. We will do this using the function createDataPartition from the caret package. The idea behind this partition is to make cross validation of our methods. 
...
## Training models 
Now that we have our final dataset, we need to train different models to see what model makes the best predictions. Since we have many variables we will use PCA analysis in order to reduce the number of variables in the model, given that many of the variables are highly correlated. We also want to reduce overfitting, for this we will perform cross validation on the training 5-fold.
...
Let´s compare the results for the different models used
...
Now we create a function to store the results
...
The random forest model provides an outstanding accuracy the results were nearly 100% in the validation and Test set.